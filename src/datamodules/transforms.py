...
def apply_reference(
    ...
    *,
    ...
    if m in ("bipolar", "bip", "bipolar_like"):
        # FIX: For CANON_CHS_18 (C==18), use the single-cycle permutation to avoid extra cycles/nullspace.
        C = _get_C(X)
        if C == 18:
            return apply_bipolar_perm(X, BIPOLAR_PERM_CANON18)

        # Fallback for other channel counts: keep previous behavior (requires neighbors).
        if lap_neighbors is None:
            raise ValueError("lap_neighbors is required for mode='bipolar' when C != 18")
...
# -------------------------
# Optional DSP helpers (cross-dataset preprocessing)
# -------------------------
...
def bandpass_filter_trials(
    ...
    *,
    ...
    """
    if butter is None:
        raise ImportError("scipy is required for bandpass_filter_trials")
    ...
    Xf = X.astype(np.float32, copy=False)
    if Xf.ndim in (2, 3):
        return sosfiltfilt(sos, Xf, axis=-1).astype(np.float32)
    raise ValueError(f"Expected 2D/3D, got {Xf.ndim}D")
...
def resample_trials(
    ...
    *,
    ...
    """Resample along time axis using polyphase filtering.
    ...
    if resample_poly is None:
        raise ImportError("scipy is required for resample_trials")
    fs_in = float(fs_in)
    fs_out = float(fs_out)
    if fs_in <= 0 or fs_out <= 0:
        raise ValueError("fs_in/fs_out must be > 0")
    ...
    Xf = X.astype(np.float32, copy=False)
    if Xf.ndim in (2, 3):
        return resample_poly(Xf, up=up, down=down, axis=-1).astype(np.float32)
    raise ValueError(f"Expected 2D/3D, got {Xf.ndim}D")